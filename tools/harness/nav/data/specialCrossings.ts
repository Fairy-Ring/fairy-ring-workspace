...
// Horror basalt Jump-across chain (basalt_rocks.rs2) — see horrorBasalt.ts
import { HORROR_BASALT_CROSSINGS } from './horrorBasalt.ts';
SPECIAL_CROSSINGS.push(...HORROR_BASALT_CROSSINGS);

/**
 * Barbarian Outpost crumbling wall — align with rs2b0t travelCatalog
 * `agi_castle_crumbling_wall` (west→east only, content climb_barbarian_crumblingwall).
 * Stands: loc 0_39_55_46_33 → world 2542,3553; from 2541,3553 → to 2543,3553.
 */
SPECIAL_CROSSINGS.push({
    x: 2541,
    z: 3553,
    level: 0,
    locName: 'Crumbling wall',
    action: 'Climb-over',
    requiresSkill: { name: 'agility', level: 5 },
    toTile: { x: 2543, z: 3553, level: 0 },
    arrivalRadius: 2,
    label: 'Outpost crumbling wall west→east (agi_castle_crumbling_wall)'
});

/**
 * Barbarian Outpost Obstacle pipe — authentic courtyard ↔ course/Gunnjorn join.
 * Loc barbarian_obstacle_pipe 2287 @ 2552,3559 (content Squeeze-through exactmove).
 * Keyed at **loc tile** so findTransportLoc + specialCrossingForTransport (locX/Z)
 * hit; toTile selects direction (N stand 2552,3561 ↔ S 2552,3558).
 * Agility 35 when entering from north (content). Do not wallgrill / wall-punch.
 */
SPECIAL_CROSSINGS.push(
    {
        x: 2552,
        z: 3559,
        level: 0,
        locName: 'Obstacle pipe',
        action: 'Squeeze-through',
        requiresSkill: { name: 'agility', level: 35 },
        toTile: { x: 2552, z: 3558, level: 0 },
        arrivalRadius: 1,
        label: 'Outpost obstacle pipe → course (south)'
    },
    {
        x: 2552,
        z: 3559,
        ...
        action: 'Squeeze-through',
        requiresSkill: { name: 'agility', level: 35 },
        toTile: { x: 2552, z: 3561, level: 0 },
        arrivalRadius: 1,
        label: 'Outpost obstacle pipe → courtyard (north)'
    }
);

/**
 * Cairn Isle rocks (Zombie Queen content `zqclimbingrocks` loc 2231).
 * Mainland E 2795,2979 ↔ island W 2791,2979 — Climb, agility 15 from east.
 * After hop, **do not** walk z=2979 x≈2776–2781 (bridge zone falls into river).
 * transports.json already has edges; specialCrossing owns settle + toTile wait.
 */
SPECIAL_CROSSINGS.push(
    {
        x: 2795,
        ...
        locName: 'Rocks',
        action: 'Climb',
        requiresSkill: { name: 'agility', level: 15 },
        toTile: { x: 2791, z: 2979, level: 0 },
        arrivalRadius: 2,
        label: 'Cairn Isle rocks → island (west)'
    },
    {
        x: 2791,
        z: 2979,
        ...
        action: 'Climb',
        requiresSkill: { name: 'agility', level: 15 },
        toTile: { x: 2795, z: 2979, level: 0 },
        arrivalRadius: 2,
        label: 'Cairn Isle rocks → mainland (east)'
    },
    // Loc tiles (findTransportLoc / specialCrossingForTransport by locX|locZ)
    {
        x: 2794,
        z: 2978,
        level: 0,
    ...
    },
    {
        ...
        toTile: { x: 2795, z: 2979, level: 0 },
        arrivalRadius: 2,
        label: 'Cairn rocks loc → mainland'
    }
);
