...
export class PathFinder {
    ...
    /**
     * Harness-only post-load tweak: set walk bit on a tile.
     * Used by `data/collisionPatches.ts` for known bake overblocks.
     */
    setWalkable(x: number, z: number, level: number, walkable: boolean): void {
        const slot = this.slotAt(x, z, level);
        if (!slot) {
            return;
        }
    ...
    /**
     * Harness-only: OR bits into exit mask (dir bits 0..7 = N E S W NE SE SW NW).
     */
    orExitMask(x: number, z: number, level: number, bits: number): void {
        const slot = this.slotAt(x, z, level);
        if (!slot) {
            return;
        }
        const index = (x & 0x3f) * 64 + (z & 0x3f);
        slot.exit[index] |= bits & 0xff;
    }

    /**
     * Harness-only: clear wall nibble bits (N=1 E=2 S=4 W=8).
     */
    clearWallBits(x: number, z: number, level: number, bits: number): void {
        const slot = this.slotAt(x, z, level);
        if (!slot || !slot.wall) {
            return;
        }
        const index = (x & 0x3f) * 64 + (z & 0x3f);
        const clear = bits & 0xf;
        if (index & 1) {
            const hi = (slot.wall[index >> 1] >> 4) & 0xf;
            const lo = slot.wall[index >> 1] & 0xf;
            slot.wall[index >> 1] = lo | (((hi & ~clear) & 0xf) << 4);
        } else {
            const hi = (slot.wall[index >> 1] >> 4) & 0xf;
            const lo = slot.wall[index >> 1] & 0xf;
            slot.wall[index >> 1] = ((hi & 0xf) << 4) | ((lo & ~clear) & 0xf);
        }
    }
    ...
    addEdges(doors: DoorEdgeData[], transports: TransportEdgeData[], stairs: TransportEdgeData[] = []): void {
        ...
        for (const edge of activated) {
            ...
            const transport: TransportInfo = {
                ...
                // Fixed landings: wait on toTile (portals / ships / agility-shortcut hops).
                // shortcut+agility: Climb-over / Squeeze etc. (rs2b0t travelCatalog + outpost hops).
                // Without toTile, same-level hops fall through to multi-tile door Open logic.
                toTile:
                    edge.kind === 'dungeon'
                    || edge.kind === 'portal'
                    || edge.kind === 'ship'
