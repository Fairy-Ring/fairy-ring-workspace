/**
 * Quest-locked door dialogue detection — port of rs2b0t exec/questLock.ts.
 * ChatDialog + Execution from harness script API.
 */

import { ChatDialog, Execution } from '../../script/browser/api.ts';
...
export function chatShowsQuestLock(): boolean {
    try {
        ...
        // Prefer option labels + modal message when available
        const texts: string[] = [...ChatDialog.options()];
        const mm = ChatDialog.modalMessage?.();
        if (mm) texts.push(mm);
        // Fall back to recent game chat if modal text empty
        if (texts.length === 0) {
            const chat = (globalThis as { __lc377?: { reader?: { chat?: (n: number) => { text?: string }[] } } })
                .__lc377?.reader?.chat?.(8);
        ...
        return isQuestLockDialogue(texts);
    } catch {
        return false;
    }
}
