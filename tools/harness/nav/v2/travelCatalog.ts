...
import { horrorBasaltTransportEdges } from '../data/horrorBasalt.ts';
...
export function agilityShortcutEdges(): TransportEdgeData[] {
    ...
    // Content: climb_barbarian_crumblingwall — start west of loc, exactmove +2 x.
    // locX/Z must be the **wall** tile (not stand) so findTransportLoc hits castlecrumbly1.
    ...
    // Barbarian outpost Obstacle pipe (barbarian_obstacle_pipe 2287 @ 2552,3559).
    // Content barbarian_course.rs2: N stand loc+(0,0,2)=2552,3561 → exactmove S −3 → 2552,3558;
    // S stand loc+(0,0,-1)=2552,3558 → exactmove N +3 → 2552,3561. Agi 35 from north only.
    // This is the authentic courtyard → Gunnjorn/course join (not wallgrill / wall punch).
    const pipeLoc = { x: 2552, z: 3559, level: 0 };
    const pipeNorth = { x: 2552, z: 3561, level: 0 };
    const pipeSouth = { x: 2552, z: 3558, level: 0 };
    ...
    return [
        {
            ...
            locX: castleLoc.x,
            locZ: castleLoc.z,
            debugName: 'agi_castle_crumbling_wall',
            options: ['Climb-over'],
            requires: membersAgi(5)
        },
        {
            from: { ...pipeNorth },
            to: { ...pipeSouth },
            ...
            locX: pipeLoc.x,
            locZ: pipeLoc.z,
            debugName: 'agi_barbarian_obstacle_pipe_s',
            options: ['Squeeze-through'],
            requires: membersAgi(35)
        },
        {
            from: { ...pipeSouth },
            to: { ...pipeNorth },
            ...
            locX: pipeLoc.x,
            locZ: pipeLoc.z,
            debugName: 'agi_barbarian_obstacle_pipe_n',
            options: ['Squeeze-through'],
            requires: membersAgi(35)
        },
...
export function curatedTravelEdges(): TransportEdgeData[] {
    return [
        ...
        // Horror basalt Jump-across (edges only load if endpoints walkable)
        ...horrorBasaltTransportEdges()
    ];
}
