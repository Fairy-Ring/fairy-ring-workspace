/**
 * Parse LostCity stairs.rs2 switch bodies into stair edges.
 * Port of rs2b0t tools/nav/stairsParse.ts (import paths adapted for harness).
 */
import type { NavPoint } from '../PathFinder.ts';

export function decodeCoord(s: string): NavPoint {
    const [level, mx, mz, lx, lz] = s.split('_').map(Number);
    return { x: mx! * 64 + lx!, z: mz! * 64 + lz!, level: level! };
}
    ...
    while ((m = blockRe.exec(text)) !== null) {
        ...
        while ((c = CASE_RE.exec(body)) !== null) {
            ...
            if (mv) {
                out.push({
                    from,
                    to: applyMovecoord(from, [Number(mv[1]), Number(mv[2]), Number(mv[3])]),
                    debugname,
                    op
                });
