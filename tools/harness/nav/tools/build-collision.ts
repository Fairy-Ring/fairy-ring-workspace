...
import CollisionEngine from '../rsmod/CollisionEngine.ts';
import { changeLandCollision, changeLocCollision, changeRoofCollision } from '../rsmod/collision.ts';
import { CollisionFlag, CollisionType } from '../rsmod/flags.ts';
import { canTravel } from '../rsmod/StepValidator.ts';

import { BLOCK_MAP_SQUARE, LEVELS, MAP_X, MAP_Z, OPEN, REMOVE_ROOFS, Reader, bridgedLevel, forEachLoc, loadLocTypes, loadMapsquares, packCoord, parseLands, zoneIndex, type LocDef } from './lib.ts';
...
function parseArgs(): { engine: string; members: boolean; out: string; verify: boolean } {
    ...
    let engine = process.env.LC377_ENGINE
        || path.resolve(import.meta.dirname, '../../../../vendor/engine');
    let members = true;
    let out = path.resolve(import.meta.dirname, '../out/collision.lcnav');
    let verifyFlag = true;
...
function main(): void {
    ...
    const f2pCsv = process.env.LC377_CONTENT
        ? path.resolve(process.env.LC377_CONTENT, 'maps/free2play.csv')
        : path.resolve(opts.engine, '../content/maps/free2play.csv');
