...
import { PathFinder, type NavPoint, type TransportEdgeData } from '../PathFinder.ts';

import { Reader, bridgedLevel, forEachLoc, loadLocTypes, loadMapsquares, parseLands } from './lib.ts';
import { parseSwitchStairs } from './stairsParse.ts';
...
const engine = path.resolve(argVal('--engine') ?? process.env.ENGINE_DIR ?? path.join(REPO_ROOT, 'vendor/engine'));
const content = path.resolve(argVal('--content') ?? process.env.CONTENT_DIR ?? path.join(REPO_ROOT, 'vendor/content'));
const out = path.resolve(argVal('--out') ?? path.join(NAV_ROOT, 'data/stairEdges.json'));
const packPath = path.resolve(argVal('--pack') ?? path.join(NAV_ROOT, 'out/collision.lcnav.gz'));
const dataDir = path.resolve(argVal('--data-dir') ?? path.join(NAV_ROOT, 'data'));
