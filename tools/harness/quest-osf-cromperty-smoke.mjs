#!/usr/bin/env node
/**
 * OSF Cromperty after Petra Search. 17→18. Name Tindel. Keep inventor. No dest.
 *
 *   WORLD_SPEED_MS=300 HARNESS_PROFILE=.tmp/playwright-osf-cromp \
 *     node tools/harness/quest-osf-cromperty-smoke.mjs
 *
 * Soft onesmallfavour 17. Stand 2683,3325 S of jm2 2683,3326 (m41_51 0 59 62: 844).
 */
import {
...
const { username, password } = resolveAccount(rest, 'osfcr');
...
async function main() {
  ...
  console.log(`[osfcr] ${base} user=${username} (Cromperty OSF)`);
  ...
  let shot = null;
  if (shotsEnabled) {
    const shotDir = await createShotRunDir(`osfcr_${username}`);
  ...
  try {
    ...
    await cheatQuiet(page, 'setvar onesmallfavour 17', 300);
    ...
    console.log('[osfcr] npc', JSON.stringify(seen));
    ...
    const talk = await page.evaluate(async () => {
      ...
      for (let i = 0; i < 180; i++) {
        ...
        if (raw.length) {
          const prefer = [/girl stuck in some rock/i, /one more .small favour/i];
          let j = -1;
    ...
    console.log('[osfcr] talk', JSON.stringify(talk));
    ...
    if (!/tindel|iron oxide|port khazard/i.test(text)) fail(`Tindel/oxide missing: ${text}`);
    ...
    if (Number(stage) !== 18) fail(`onesmallfavour=${stage} (want 18)`);

    console.log(`RESULT PASS osfcr Cromperty 17→18 stay house user=${username}`);
    process.exit(0);
  } catch (e) {
    console.error(e);
