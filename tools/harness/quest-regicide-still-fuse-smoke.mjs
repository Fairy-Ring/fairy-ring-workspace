#!/usr/bin/env node
/**
 * Regicide still → fuse — no give naphtha, no give fused.
 *
 *   WORLD_SPEED_MS=300 node tools/harness/quest-regicide-still-fuse-smoke.mjs
 *
 * Soft: stage 11 + upass 10 + tar/coal/raw mats.
 * Product: still (P green) → naphtha 3221 → grind/mix/loom/fuse → 3219.
 *
 * @see docs/plans/2026-08-13-regicide-still-fuse.md
 */
import {
...
const { username, password } = resolveAccount(rest, 'regsf');
...
async function main() {
  ...
  console.log(`[reg-sf] ${base} user=${username} (still→fuse; headed default)`);
  ...
  let shot = null;
  if (shotsEnabled) {
    const shotDir = await createShotRunDir(`reg-sf_${username}`);
  ...
  try {
    ...
    console.log(
      `RESULT PASS reg-sf still+fuse naphtha ${NAPHTHA_ID} → fused ${FUSED_ID} (SOFT stage 11; no give naphtha/fused)`
    );
    process.exit(0);
  } catch (e) {
    console.error(e);
