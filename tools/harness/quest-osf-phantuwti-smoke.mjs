#!/usr/bin/env node
/**
 * OSF Phantuwti after Arhein. 15→16. Name Petra. No dest / weather grant.
 *
 *   WORLD_SPEED_MS=300 HARNESS_PROFILE=.tmp/playwright-osf-phan \
 *     node tools/harness/quest-osf-phantuwti-smoke.mjs
 *
 * Soft onesmallfavour 15. Stand 2704,3473 SW of jm2 2705,3474 (m42_54 0 17 18: 1798).
 */
import {
...
const { username, password } = resolveAccount(rest, 'osfph');
...
async function main() {
  ...
  console.log(`[osfph] ${base} user=${username} (Phantuwti OSF)`);
  ...
  let shot = null;
  if (shotsEnabled) {
    const shotDir = await createShotRunDir(`osfph_${username}`);
  ...
  try {
    ...
    await cheatQuiet(page, 'setvar onesmallfavour 15', 300);
    ...
    console.log('[osfph] npc', JSON.stringify(seen));
    ...
    const talk = await page.evaluate(async () => {
      ...
      for (let i = 0; i < 180; i++) {
        ...
        if (raw.length) {
          const prefer = [/what can i do to help/i, /yes, ok, i'll do it/i];
          let j = -1;
    ...
    console.log('[osfph] talk', JSON.stringify(talk));
    ...
    if (!/petra|south westerly|caves/i.test(text)) fail(`Petra/caves missing: ${text}`);
    if (/12346|remove this key|lamp|weather report grant|cromperty/i.test(text)) fail(`later leftover leaked: ${text}`);

    const tile = await worldTile(page);
    console.log('[osfph] tile', tile);
    ...
    if (Number(stage) !== 16) fail(`onesmallfavour=${stage} (want 16)`);

    console.log(`RESULT PASS osfph Phantuwti 15→16 stay Seers user=${username}`);
    process.exit(0);
  } catch (e) {
    console.error(e);
