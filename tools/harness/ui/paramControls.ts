/**
 * Settings control widgets for ParamsModal — port of rs2b0t paramControls
 * (tile control uses number fields only; map pick is already open as host).
 */
import { settingOptionLabel, type SettingDef, type SettingsSchema } from '../runtime/Settings.ts';
...
export function visibilityDeps(schema: SettingsSchema): Set<string> {
    ...
    for (const def of Object.values(schema)) {
        if (def.showIf) deps.add(def.showIf.key);
    }
    return deps;
}
        ...
        edit(_def, current, onChange, { disabled }) {
            ...
            (['x', 'z', 'lvl'] as const).forEach((name, i) => {
                ...
                inp.addEventListener('change', () =>
                    onChange(fields.map(x => x.value.trim() || '0').join(','))
                );
                fields.push(inp);
                f.appendChild(inp);
                wrap.appendChild(f);
