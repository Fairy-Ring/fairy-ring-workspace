/**
 * Port of rs2b0t BankChapel stages — state-driven.
 */
import {
    type Task,
    Execution,
...
class UseBankBooth extends StageTask {
    ...
    async execute(): Promise<void> {
        ...
        await Execution.delayUntil(() => !noDialog() || Bank.isOpen(), 8000);
        if (Bank.isOpen()) this.progress.bankOpened = true;
    }
}
...
class OpenPrayerTab extends StageTask {
    ...
    async execute(): Promise<void> {
        if (await Game.openSideTab(PRAYER_TAB)) this.opened = true;
    }
}
...
class OpenFriendsTab extends StageTask {
    ...
    async execute(): Promise<void> {
        if (await Game.openSideTab(FRIENDS_TAB)) this.opened = true;
    }
}
...
class OpenIgnoreTab extends StageTask {
    ...
    async execute(): Promise<void> {
        if (await Game.openSideTab(IGNORE_TAB)) this.opened = true;
    }
}
