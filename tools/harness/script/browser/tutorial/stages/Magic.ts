/**
 * Port of rs2b0t Magic stages — state-driven.
 */
import {
    type Task,
    Execution,
...
class OpenMagicTab extends StageTask {
    ...
    async execute(): Promise<void> {
        if (await Game.openSideTab(MAGIC_TAB)) this.opened = true;
    }
}
...
class FinishTutorial extends StageTask {
    ...
    async execute(): Promise<void> {
        ...
        await npc.interact('Talk-to');
        await Execution.delayUntil(() => !noDialog(), 8000);
    }
}
...
export function magicStages(bot: TutorialBot): Task[] {
    ...
    return [
        new TalkTerrova(bot),
        new OpenMagicTab(bot),
        new TalkForRunes(bot, progress),
        new CastWindStrike(bot, progress),
        new FinishTutorial(bot, progress)
    ];
}
