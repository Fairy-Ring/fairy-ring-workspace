# Docs (contributor surface)

This tree ships a **thin** documentation set only.

| Included | Not included by default |
|----------|-------------------------|
| Decisions (fences, branding, thin surface) | Session thrash plans |
| Authenticity stance | Full readiness XL / port dumps |
| **Deviations** + **softpass** ledgers | Gap dumps / private cold-start notes |
| Root `AGENT_BRIEF.md` | Harness screenshot archives |
| Thin runbooks | |

Need depth on a **named** unit? Open a GitHub issue — Decision **011**.

## Runbooks (this surface)

| Runbook | Use |
|---------|-----|
| [`runbooks/isolation.md`](../runbooks/isolation.md) | Ports, isolation from other stacks |
| [`runbooks/smoke-start.md`](../runbooks/smoke-start.md) | Engine start checklist |
| [`runbooks/vendor-layout.md`](../runbooks/vendor-layout.md) | Clone layout for content/engine/client |
| [`runbooks/harness.md`](../runbooks/harness.md) | Playwright thrash / smokes |
| [`runbooks/client-ts.md`](../runbooks/client-ts.md) | TS web client build/deploy |
| [`runbooks/playable.md`](../runbooks/playable.md) | Java client against this engine |
